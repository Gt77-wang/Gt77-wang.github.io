...
const CAR_SPEED_MS = 2.5;                // 2.5 m/s
const BATTERY_FULL = 100;                // %
const BATTERY_DRAIN_PER_METER = 0.002;   // 0.002%/m → 500m loop ≈ 1%
const BATTERY_DRAIN_PER_WAYPOINT = 0.15;  // 0.15% per stop
const SPEED_DISP_MS = 500;               // throttle speed display
const BATT_DISP_MS = 2000;               // throttle battery display
let lastSpeedDisp = 0;
let lastBattDisp = 0;
...
// MOCK: 模拟通信模块 — 替换为真实 MQTT/WebSocket 时修改此处
let gpsConnected = false;          // 通信连接状态
let gpsCarLat = null;             // 最后一次收到的小车纬度
let gpsCarLng = null;             // 最后一次收到的小车经度
let gpsTargetLat = null;          // 目标地点纬度
let gpsTargetLng = null;          // 目标地点经度
let gpsReceiveCount = 0;          // 接收数据包计数
let gpsSendCount = 0;             // 发送数据包计数
let gpsReceiveInterval = null;    // 模拟 GPS 接收定时器
let gpsTargetMarker = null;       // 目标地点地图标记
let gpsCarGpsMarker = null;       // 小车 GPS 上报位置标记 (区别于动画位置)
let gpsMockIndex = 0;             // 模拟 GPS 数据游标
 ...
 * 连接小车 GPS 通信
 * MOCK: 模拟建立连接。实际替换为:
 *   - WebSocket: new WebSocket('ws://car-ip:8080/gps')
 *   - MQTT: client.connect('broker-url', {clientId})
 *   - HTTP: 开始轮询 fetch('/api/gps/latest')
 */
function gpsConnect() {
  if (gpsConnected) return;

  // MOCK: 模拟连接成功 (延迟 300ms)
  setTimeout(() => {
    gpsConnected = true;
    gpsReceiveCount = 0;
    gpsSendCount = 0;
    ...
    // MOCK: 开始模拟接收 GPS 数据 (每 2 秒一次)
    // 实际替换为: WebSocket.onmessage / MQTT on('message') / setInterval(fetch)
    gpsReceiveInterval = setInterval(gpsSimulateReceive, 2000);
    gpsSimulateReceive(); // 立即接收一次
  }, 300);
 ...
 * 模拟接收小车 GPS 数据
 * MOCK: 根据小车运行状态生成模拟坐标。
 * 实际替换为: 从 WebSocket/MQTT/HTTP 获取真实 GPS 数据
 */
function gpsSimulateReceive() {
  ...
  // MOCK: 实际替换时，这里接收的是小车上传的真实坐标
  // 例如: WebSocket onmessage → JSON.parse → gpsHandleReceive(lat, lng)
 ...
 * 发送目标地点经纬度给小车
 * MOCK: 模拟发送，延时后显示确认。
 * 实际替换为: WebSocket.send() / MQTT publish() / fetch POST
 */
function gpsSendTarget() {
  ...
  // MOCK: 模拟发送 (实际替换为 WebSocket/MQTT/HTTP 发送)
  // ============================================================
  // 实际代码示例:
  //   WebSocket: ws.send(JSON.stringify({type:"target", lat, lng}));
  //   MQTT:      client.publish('car/target', JSON.stringify({lat, lng}));
  //   HTTP:      fetch('/api/target', {method:'POST', body:JSON.stringify({lat,lng})});
  // ============================================================
